...
const sendVerificationEmail = async (email, verificationCode, otherDetails) => {
    const transporter = nodemailer.createTransport({
        ...
        auth: {
            ...
            pass: config.emailSender.SENDER_PASSWORD,
        },
    });

    const mailOptions = {
        from: config.emailSender.SENDER_EMAIL,
        to: email,
...
registerCustomersCtrl.registerCustomer = async (req, res) => {
    ...
    try {
        ...
        const newCustomer = new customersMdl({
            ...
            active: otherDetails.active || false
        });
